...
namespace Five9AzureSpeech2Text.Services
{
    public class Five9VoiceService : Voice.VoiceBase
    {
        ...
        public override async Task StreamingVoice(IAsyncStreamReader<StreamingVoiceRequest> requestStream, IServerStreamWriter<StreamingVoiceResponse> responseStream, ServerCallContext context)
        {
            ...
            if (await requestStream.MoveNext(context.CancellationToken))
            {
                if (requestStream.Current.StreamingConfig != null)
                {
                    ...
                    _logger.LogInformation($"StreamingConfig.AgentId: {requestStream.Current.StreamingConfig.AgentId}");
            ...
            SubsribeToRecognizerEvents(transcriber, stopRecognition, startRecognition);

            await transcriber.StartTranscribingAsync(); //.StartContinuousRecognitionAsync();
            ...
            await transcriber.StopTranscribingAsync();//.StopContinuousRecognitionAsync();
        ...
        private void SubsribeToRecognizerEvents(ConversationTranscriber transcriber, TaskCompletionSource<int> stopRecognition, TaskCompletionSource<int> startRecognition)
        {
            ...
            {
                _logger.LogInformation($"TRANSCRIBING: Text={e.Result.Text} Speaker ID={e.Result.SpeakerId}");
            };
            ...
            transcriber.Transcribed += (s, e) =>
            {
                ...
                {
                    _logger.LogInformation($"TRANSCRIBED: Text={e.Result.Text} Speaker ID={e.Result.SpeakerId}");
                }
